...
The Prompt Processing system will receive images from both LATISS and LSSTCam, and run one of several pipelines on them, depending on the purpose of the observations.
In the baseline case (running the full Alert Production pipeline on images for the Large Survey of Space and Time), the system will create multiple outputs: the transient alert stream, the alert production database (APDB), datasets in a central repository, and Sasquatch metrics for pipeline analytics and debugging.

As of May 2023, the Prompt Processing system starts processing when it receives a ``next_visit`` event from the summit before observations are taken.
...
When all exposures have arrived, the worker runs the pipeline, updating the APDB and sending alerts as appropriate; if not all exposures arrive, it attempts to process what it has.
Once pipeline processing is complete, the worker uploads its outputs to the central repository.
...
Even within a single worker, the order of events is not fully deterministic (e.g., two snaps could be ingested and processed up to snap combination in parallel).
However, the possible failures can be divided into a handful of cases:

- Any failure before the end of APDB writing (which is atomic as of May 2023) has no consequences beyond that worker's local repository.
...
- A failure after alert distribution but but before or during central repo sync may leave the latter missing calexps and difference images, which are supposed to be exported to the Rubin Science Platform.
...
For example, if DiaSource and DiaObject IDs are deterministic functions of only their visit, then pipeline code might handle retries by testing for these IDs in the APDB, and ignoring or overwriting them.
...
The expectation of idempotency (see :ref:`general-priorities`) amounts to making the association results independent of the processing order.
...
As of May 2023, this is also the order in which the pipeline produces outputs.
...
The alert stream can be easily restored from the APDB, if we have a record of which alerts were sent or can tolerate duplicates.
...
The former carries the risk that the worker will be left in an inconsistent state, corrupting future processing, while the latter loses all intermediate data and adds the overhead of starting a new worker and preparing its local repository.
